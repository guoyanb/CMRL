...
class BayesianFeatureLearning(nn.Module):
    ...
    def forward(self, features):
        ...
        for node_type in ['g', 'm', 'd']:
            ...
            pi = self.mixture_weights[node_type](feat)
            ...
            posterior_mu = posterior_params[:, :, :, 0]
            posterior_log_sigma = posterior_params[:, :, :, 1]
            posterior_sigma = torch.exp(0.5 * posterior_log_sigma)

            samples = self.reparameterize(posterior_mu, posterior_log_sigma)

            weighted_samples = torch.sum(pi.unsqueeze(-1) * samples, dim=1)
...
class CMRL(nn.Module):
    def __init__(self, device, meta_paths, test_data, in_size, hidden_size, num_heads, dropout, etypes):
        ...
        self.layers1 = CMRL_Layer(self.device, meta_paths, test_data, hidden_size, r_vec, num_heads, dropout, etypes,
                                    name=['g', 'm', 'd'])
    ...
    def forward(self, g, features, data):
        inputs = {key: value.to(self.device) for key, value in features.items()}
...
class MessageAggregator(nn.Module):
    ...
    def forward(self, nodes, metapath_instances, metapath_embedding, features):
        ...
        for i in range(len(nodes)):
            ...
            if index:
                node_metapath_embedding = metapath_embedding[index]
                ...
                node_embedding = torch.vstack([features[i]] * len(index))
